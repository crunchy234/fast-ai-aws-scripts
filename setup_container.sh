...
export awsKey=`aws ec2 create-key-pair --key-name "aws-key-$(get_unique_id)" --query 'KeyMaterial' --output text`

if [ -n "$awsKey" ]
then
    rm -f ~/.ssh/aws-key.pem || true
    echo "$awsKey" > ~/.ssh/aws-key.pem
    chmod 400 ~/.ssh/aws-key.pem
fi


export instanceId=`aws ec2 run-instances --image-id $imageId --count 1 --instance-type $instanceType --key-name "aws-key-$(get_unique_id)" --security-group-ids $securityGroupId --subnet-id $subnetId --associate-public-ip-address --block-device-mapping "[ { \"DeviceName\": \"/dev/sda1\", \"Ebs\": { \"VolumeSize\": 128, \"VolumeType\": \"gp2\" } } ]" --query 'Instances[0].InstanceId' --output text`
...
echo vpcId: $vpcId
echo interretGatewayId: $internetGatewayId
...
echo "$(get_script_dir)/resume_container.sh $instanceId $instanceType" >> $resumeCurrentFileName
chmod +x $resumeCurrentFileName

# Create termination script
terminateCurrentFileName="$(get_script_dir)/terminateCurrent.sh"
terminateStr="$(get_script_dir)/terminate_container.sh $instanceId $vpcId $internetGatewayId $subnetId $securityGroupId $routeTableId"
...
echo "rm -- $instanceUrlFile" >> $terminateCurrentFileName
